...
static void
TI2_print_exports (FILE *fp, sTI2TypLib *tl)
{
  ...
  if (!no)
    return;
  tlbname = (char *) malloc (strlen (tl->name) + 5);
  strcpy (tlbname ,tl->name);
  h = strchr (tlbname, '.');
  if (h != NULL)
  {
    do {
...
void
TI2_typlib_idl (FILE *fp, sTI2TypLib *tl)
{
  int befirst;
  if (!tl)
    return;
  fprintf (fp, "/* Automated generated idl file <%s>.\n"
    " * Generated by genidl tool (c) 2009 Mingw-w64 project.\n"
    " */\n\n", (tl->name ? tl->name : "unknown"));
  TI2_print_exports (fp, tl);
