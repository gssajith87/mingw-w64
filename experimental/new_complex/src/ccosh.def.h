...
__FCT_ABIEXT(ccosh) (__FCT_TYPE __complex__ z)
{
  ...
  if (r_class != FP_NAN && r_class != FP_INFINITE)
  {
    if (i_class != FP_NAN && i_class != FP_INFINITE)
    {
      ...
      __real__ ret = __FCT_ABIEXT(cosh) (__real__ z) * c_x;
      __imag__ ret = __FCT_ABIEXT(sinh) (__real__ z) * s_x;
    }
    else
    {
      __imag__ ret = __real__ z == __FCT_CSTEXT(0.0) ? __FCT_CSTEXT(0.0) : __FCT_NAN;
      __real__ ret = __FCT_NAN + __FCT_NAN;
    }
    return ret;
  }
  ...
  if (r_class == FP_INFINITE)
  {
    if (i_class == FP_ZERO)
    {
      ...
      __imag__ ret = __imag__ z * __FCT_ABIEXT(copysign) (__FCT_CSTEXT(1.0), __real__ z);
    }
	...
	* __FCT_ABIEXT(copysign) (__FCT_CSTEXT(1.0), __real__ z));
    }
    else
    {
      __real__ ret = __FCT_HUGE_VAL;
